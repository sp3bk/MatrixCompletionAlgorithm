...
g34 = list() 
for ( i in 1:10 ) {
  g34[i] <- append(g34[i],list())
} 
#####################################################
#NEW:Gaining successrates for different RANDOM
count=2 #number of loops 
for (j in 1:count) {
  for (i in 1:10) {
    
    x <- graphx(x.ad, x2,y2,xbar1, xbar2, ybar1,ybar1p, MaxNumColors,warmcolor, coldcolor, (10-i)/10.0) #Note: 0.1 = 90% random
    seednumber <- Sys.time() 
    seednumber <- gsub(':','',seednumber) 
    seednumber <- gsub('-','',seednumber) 
    seednumber <- gsub(' ','',seednumber) 
    ...
    softimpute(seednumber)
    b=graph(x.ad, "x2.csv", x2, y2,xbar1,xbar2,ybar1, ybar1p,MaxNumColors,warmcolor, coldcolor)
    gx3 <- c(gx3, 100-(i*10.0))
    
    s <-successrate(x, a, b)
    gy3 <- c(gy3, s) 
    g34[[i]] <- append(g34[[i]],s) 
    
  }
}
############################################################################
#Compiling and saving datas for RANDOM
data.raw2 <- readRDS("compile100b.Rda")
...
saveRDS(data.raw,file="compile100b.Rda")

gx2 <- readRDS("compilex100b.Rda")
gy2 <- readRDS("compiley100b.Rda")


gx3 <- append(gx2, gx3)
gy3 <- append(gy2, gy3)

saveRDS(gx3, file="compilex100b.Rda")
saveRDS(gy3, file="compiley100b.Rda")

#Obtaining sd, mean, etc. for RANDOM
data.summary <- data.frame(
  Random=levels(data.raw$group),
...
#Graphing scatterplot with linear regresson for RANDOM
par(fig=c(0,x2,0,y2), new=FALSE)

plot(gx3, gy3, pch = 5, cex = 1, col = "blue", main = "SVD- Random vs. Success Rate .", xlab = "Random (%)", ylab = "Success Rate (%)")
abline(lm(gy3 ~ gx3))

#Error bars for RANDOM
par(fig=c(0,x2,0,y2), new=FALSE)
ggplot(data.summary, aes(x = Random, y = Success_Rate)) +  
  geom_point(shape=1) +
  geom_abline() +
  geom_errorbar(aes(ymin=Success_Rate-sd, ymax=Success_Rate+sd)) +
  ggtitle("Bar plot with standard deviation as error bars")
##################################################################
#Gaining successrates for different NOISE
count=2 #number of loops 
for (j in 1:count) {
  for (i in 1:10) {
...
##############################################################################
#Compiling and saving datas for NOISE
data.raw2 <- readRDS("compile100.Rda")
data.raw <- data.frame(group = rep(c('99','90','80','70','60','50','40','30','20','10'),each=count),data = c(g[[1]],g[[2]],g[[3]],g[[4]],g[[5]],g[[6]],g[[7]],g[[8]],g[[9]],g[[10]]) )
data.raw <- rbind(data.raw, data.raw2)
#data.raw <- append(data.raw, data.raw2)
saveRDS(data.raw,file="compile100.Rda")

gx2 <- readRDS("compilex100.Rda")
gy2 <- readRDS("compiley100.Rda")


gx <- append(gx2, gx)
gy <- append(gy2, gy)

saveRDS(gx, file="compilex100.Rda")
saveRDS(gy, file="compiley100.Rda")

#Obtaining sd, mean, etc. for NOISE
data.summary <- data.frame(
...
#Graphing scatterplot with linear regresson for NOISE
par(fig=c(0,x2,0,y2), new=FALSE)
